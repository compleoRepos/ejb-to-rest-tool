...
public class SmartCodeEnhancer implements CodeEnhancer {
        ...
        if (Files.exists(propsFile)) {
            ...
            if (!content.contains("spring.profiles")) {
                String profileConfig = """
                        ...
                        spring.profiles.group.dev=mock
    ...
    private void applyTestingRules(EnhancementReport report, Path testDir,
        ...
        for (UseCaseInfo uc : analysisResult.getUseCases()) {
            // Guard : en mode BIAN (ACL), les tests sont deja generes par AclArchitectureGenerator
            // (PaymentOrderControllerTest + ConnectivityTest). Ne pas generer de tests individuels
            // par UseCase car les controllers individuels n'existent plus.
            if (uc.getBianMapping() != null) {
                log.info("[R69] Skip SmartCodeEnhancer test for {} : BIAN mode active, tests generated by AclArchitectureGenerator",
            ...
            // Guard : ne pas generer de test si les noms de DTO sont null
            if (uc.getInputDtoClassName() == null || uc.getOutputDtoClassName() == null) {
                log.warn("[R69] Skip test generation for {} : inputDto={}, outputDto={}",
                        uc.getControllerName(), uc.getInputDtoClassName(), uc.getOutputDtoClassName());
                continue;
            }
            ...
            if (!Files.exists(testFile)) {
                ...
                String testCode = """
                        ...
                        import %s.dto.request.%s;
                        import %s.dto.response.%s;
                        import %s.domain.service.%s;
