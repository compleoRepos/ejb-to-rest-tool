...
const PROJECT_DIR = "/tmp/nexabank-core-real/nexabank-core";
...
async function main() {
  ...
  // Pattern 8: JMS @MessageDriven → @JmsListener
  {
    ...
    // v12.5: Exclude migration-notes (documentation) from the check
    const javaFiles = allFiles.filter(f => f.path.endsWith(".java"));
    const javaContent = javaFiles.map(f => f.content).join("\n");
    const hasMessageDriven = javaContent.includes("@MessageDriven");
  ...
  {
    const hasJmsTemplate = allGeneratedContent.includes("JmsTemplate") || allGeneratedContent.includes("jmsTemplate") || allGeneratedContent.includes("KafkaTemplate") || allGeneratedContent.includes("kafkaTemplate");
  ...
  {
    // v12.5: Check for Multi-DataSource Configuration section in application.yml
    const hasMultiDsConfig = allGeneratedContent.includes("Multi-DataSource Configuration");
    const dsNames = allGeneratedContent.match(/(loan|ledger|swift)/gi) || [];
    const uniqueDsNames = new Set(dsNames.map((n: string) => n.toLowerCase()));
    const dsCount = uniqueDsNames.size;
    const hasConfig = allFiles.some(f => f.path.toLowerCase().includes("application.yml"));
    const status = hasMultiDsConfig && dsCount >= 2 ? "OK" : dsCount >= 1 ? "PARTIAL" : "KO";
    audits.push({
      id: 12, pattern: "Multi-DataSource (3 DS)",
      status, score: status === "OK" ? 5 : status === "PARTIAL" ? 3 : 0,
      details: `multiDsConfig=${hasMultiDsConfig}, uniqueDsNames=${dsCount} (${[...uniqueDsNames].join(",")}), hasConfig=${hasConfig}`,
    });
  }
  ...
  {
    // v12.5: Exclude TransactionAspectSupport.currentTransactionStatus().setRollbackOnly() (correct Spring pattern)
    const hasOldRollback = (allGeneratedContent.match(/(?:sctx|sessionContext|ctx)\.setRollbackOnly/g) || []).length > 0;
