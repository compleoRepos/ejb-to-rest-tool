...
function generateStateEnum(
  ...
  // Générer les commentaires pour chaque état
  const stateComments: Record<string, string> = {
    INITIATED: "État initial — la Saga vient d'être créée, aucun step n'a encore été exécuté",
    COMPLETED: "Tous les steps ont été exécutés avec succès — état terminal positif",
    COMPENSATING: "Un step a échoué, la compensation LIFO est en cours",
    COMPENSATED: "Toutes les compensations ont réussi — l'état cohérent est restauré (terminal)",
    FAILED: "Un step a échoué et la compensation n'a pas été tentée (terminal)",
    COMPENSATION_FAILED: "Au moins une compensation a échoué après retry — ALERTE CRITIQUE, intervention manuelle requise (terminal)",
  };

  const stateLines = states.map((s, i) => {
    const comment = stateComments[s] || `Step ${s.replace(/^STEP_/, '')} en cours d'exécution`;
    return `    /** ${comment} */\n    ${s}${i < states.length - 1 ? "," : ";"}`;
  }).join("\n\n");
 ...
 *       └──► COMPENSATION_FAILED  (au moins 1 compensation échouée → Dead Letter)
 * </pre>
 *
 * Règles :
 * - Seuls les états terminaux (COMPLETED, COMPENSATED, FAILED, COMPENSATION_FAILED)
 *   marquent la fin de la Saga.
 * - COMPENSATING est un état transitoire : la Saga parcourt les steps complétés
 *   en ordre LIFO (Last In, First Out) pour annuler les effets.
 * - COMPENSATION_FAILED déclenche l'envoi vers la Dead Letter Queue pour
 *   intervention manuelle ou recovery automatique.
 *
...
${stateLines}
    ...
    /**
     * Indique si cet état est terminal (la Saga ne progressera plus).
     * Les états terminaux sont : COMPLETED, FAILED, COMPENSATED, COMPENSATION_FAILED.
     * Utilisé par le SagaRecoveryScheduler pour identifier les Sagas orphelines
     * (non-terminales depuis plus de 30 minutes).
     */
    ...
    /**
     * Indique si la Saga est en cours de compensation.
     * Pendant la compensation, les steps sont annulés en ordre LIFO
     * avec RetryPolicy.forCompensation() (5 tentatives, backoff 3x).
     */
