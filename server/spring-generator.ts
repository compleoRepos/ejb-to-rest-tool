...
`,
  ...
  for (const [domain, useCases] of domainMap) {
    const serviceFile = generateDomainService(basePackage, basePath, domain, useCases, dtoMap, ir);
    files.push(serviceFile);

    // v12.10: Generate Repository interface if service uses repository pattern
    if (/\brepository\./.test(serviceFile.content)) {
      // Detect entity name from the service content
      let entityName = "";
      // Strategy 1: List<Entity> pattern
      const listMatch = serviceFile.content.match(/List<(\w+)>/);
      if (listMatch && listMatch[1] !== "String" && listMatch[1] !== "Object") {
        entityName = listMatch[1];
      }
      // Strategy 2: Scan UseCases rawSource for em.find patterns
      if (!entityName) {
        for (const uc of useCases) {
          if (uc.rawSource) {
            const emFindMatch = uc.rawSource.match(/(?:em|entityManager)\.find\s*\(\s*(\w+)\.class/);
            if (emFindMatch) { entityName = emFindMatch[1]; break; }
            ...
            if (persistMatch) {
              ...
              if (typeMatch && typeMatch[1] !== "var" && typeMatch[1] !== "Object") {
                entityName = typeMatch[1]; break;
              }
            }
          }
        }
      }
      // Strategy 3: Domain-based fallback
      if (!entityName) {
        entityName = domain.charAt(0).toUpperCase() + domain.slice(1) + "Entity";
      }
      const repoName = `${entityName}Repository`;
      const repoPath = `${basePath}/repository/${repoName}.java`;
      // Only generate if not already generated
      if (!files.some(f => f.path === repoPath)) {
        // Also generate stub entity if it doesn't exist yet
        const entityPath = `${basePath}/entity/${entityName}.java`;
        if (!files.some(f => f.path === entityPath) && !_knownEntityNames.has(entityName)) {
          files.push({
            path: entityPath,
            category: "entity",
...
/**
 * ${entityName} — JPA entity stub.
 * Auto-generated by Compleo Modernizer. Complete with actual fields.
 */
@Entity
@Data
public class ${entityName} {
    ...
    // TODO: Add entity fields from legacy model
}
`,
          });
          _knownEntityNames.add(entityName);
        }
        files.push({
          path: repoPath,
          category: "repository",
...
/**
 * ${repoName} \u2014 Spring Data JPA repository for ${entityName}.
 * Auto-generated by Compleo Modernizer (v12.10 EntityManager migration).
 */
@Repository
