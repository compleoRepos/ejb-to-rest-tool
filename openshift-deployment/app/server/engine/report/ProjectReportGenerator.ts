...
function extractTodoCards(input: ReportInput): TodoCard[] {
  ...
  for (const err of errors) {
    ...
    todos.push({
      ...
      severity,
      severityClass: classification.todoCategory === "framework-dependency" ? "framework" : (severity === "low" ? "low" : severity === "medium" ? "medium" : ""),
...
<ul>
<li>Fichiers générés : ${safeGet(() => String(input.generatedProject?.files.length ?? "N/A"), "N/A")}</li>
<li>Erreurs compilation : ${safeGet(() => String(input.compilationResult?.finalErrors?.length ?? "N/A"), "N/A")}</li>
<li>Use cases : ${safeGet(() => String(input.ir?.useCases?.length ?? "N/A"), "N/A")}</li>
</ul>
</body></html>`;
